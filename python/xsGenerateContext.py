        ...
        if len(sys.argv) >1:
            for arg in sys.argv[1:]:
                if arg.lower().startswith("avg="):
                    ...
                    errString = "xsGenerateContext-F-003: Invalid argument: " + arg
                    raise Exception(errString) 
        else:
            raise Exception("xsGenerateContext-F-001: Usage: xsGenerateContext name=<string> terms=<semantictermlist-option> (type=<contexttype-option>)? (<fuzzyvalues-option>)*")

        if avg == '':
            raise Exception("xsGenerateContext-F-003: parameter 'avg' not found")
        if count == '':
            raise Exception("xsGenerateContext-F-005: parameter 'count' not found")
        if max == '':
            raise Exception("xsGenerateContext-F-007: parameter 'max' not found")
        if min == '':
            raise Exception("xsGenerateContext-F-009: parameter 'min' not found")
        if stdev == '':
            raise Exception("xsGenerateContext-F-011: parameter 'stdev' not found")
