/*
 ...
 Program: xsAggregateSpearmanCorrelation

 Usage: xsAggregateSpearmanCorrelation [-f output_file]

 Description:
        Aggregates (combines) Spearman correlation algorithms that have the same X field, Y field,
        BY field and BY value into a new algorithm. Takes input from the output of previous xsPerform
        commands. The algorithms are matched by field and a weighted average (by count) is
        used to aggregate the results. This can be used to aggregate the results of minor periods into
        more major periods. An effective way to specify the input data stream is to pipe the output of
        inputlookup.
...
int main(int argc, char* argv[]) 
{
    ...
    // Load the Splunk header information 
