...
int main(int argc, char* argv[]) 
{
    ...
    while ((c = getopt(argc, argv, "b:ix:y:")) != -1) 
    {
        switch(c)
        {
            ...
            case 'y':
                ...
                break;
            case '?':
                fprintf(stderr, "xsprecorrelate-F-101: Unrecognised option: -%c\n", optopt);
                argError = true;
        }
    }
    if (argError)
    ...
    {
        fprintf(stderr, 
                "xsprecorrelate-F-105: found %d X and %d Y parameters.  Values must be the same\n",
                numXAxis, numYAxis);
        exit(EXIT_FAILURE);
    }
    ...
    {
        fprintf(stderr,
                "xsprecorrelate-F-111: found %d X and %d B parameters.  Values must be the same\n",
                numXAxis, numBAxis);
        exit(EXIT_FAILURE);
    }
    ...
    for(i=0; i<numXAxis; i++)
    {
        ...
        {
            if (mustMatchFields == true)
                fprintf(stderr, "xsprecorrelate-F-107: x not found: %s\n", xList[i]);
            missedOne = true;
        }
        if (yFieldIndex[i] == -1)
        {
            if (mustMatchFields == true)
                fprintf(stderr, "xsprecorrelate-F-109: y not found: %s\n", yList[i]);
            missedOne = true;
        }
    }
