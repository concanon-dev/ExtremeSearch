/*
 ...
 Program: xsAggregateQuadRegression

 Usage: xsAggregateQuadRegression [-f output_file]

 Description:
        Aggregates (combines) quadratic regression algorithms that have the same X field, Y field, BY field
...
int main(int argc, char* argv[]) 
{
    ...
    while ((c = getopt(argc, argv, "f:")) != -1) 
    {
        switch(c)
        {
            case 'f':
                ...
                break;
            case '?':
                fprintf(stderr, "xsAggregateQuadRegression-F-101: Unrecognised option: -%c\n", optopt);
                argError = true;
        }
    }
    if (argError == true)
    {
        fprintf(stderr, "xsAggregateQuadRegression-F-103: Usage: xsAggregateQuadRegression\n");
        exit(0);
    }
    ...
    // if the outfile is specified, open it 
    char tempDir[512];
    FILE *f = NULL;
    if (outfile[0] == '\0')
    {
