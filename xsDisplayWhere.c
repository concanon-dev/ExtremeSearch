/*
 ...
 Program: xsDisplayWhere

 Usage: xsDisplayWhere [-a alfacut] [-c cixFunction] [-n cixName] [-p scalar_percent] [-s synonymsFile] -w whereLine
 ...
 Description:
    Display the concepts generated by a xsWhere query.
*/
#include <errno.h>
#include <libgen.h>
#include <math.h>
...
int main(int argc, char* argv[]) 
{
    ...
    while ((c = getopt(argc, argv, "a:c:n:p:s:w:")) != -1) 
    {
        switch(c)
        {
            ...
            case 'w':
                ...
                break;
            case '?':
                fprintf(stderr, "xsDisplayWhere-F-101: Unrecognised option: -%c\n", optopt);
                argError = true;
        }
    }
    if (argError)
    {
        fprintf(stderr, "xsDisplayWhere-F-103: Usage: xsDisplayWhere [-a alfacut] [-c cixFunction] [-n cixName] [-p scalar_percent] [-s synonymsFile] -w whereLine\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (p == NULL)
    {
        fprintf(stderr, "xsDisplayWhere-F-105: Can't get header info\n");
        exit(EXIT_FAILURE);
    }

    if (saSplunkReadInfoPathFile(p) == false)
    {
        fprintf(stderr, "xsDisplayWhere-F-107: Can't read search results file %s\n",
    ...
    if (stackSize < 0)
    {
        fprintf(stderr, "xsDisplayWhere-F-111: parse error: %s\n", tempbuf);
        exit(EXIT_FAILURE);
    }
    ...
    for(i=0; i<stackSize; i++)
    {
        if (expStack[i]->type == SA_TOKEN_FIELD)
        {
            ...
            if (k == numHeaderFields)
            {
                ...
                fprintf(stderr, "xsDisplayWhere-F-113: Field %s does not exist\n",
                        expStack[i]->field);
                exit(EXIT_FAILURE);
            }
        }
